...
| **Maximum output tokens**      | Caps completion tokens, including reasoning tokens for many reasoning models. | `1200`          | Increase to `4096` or more for long reports or reasoning models. The allowed range is 64–32768.                             |
...
If a reasoning model consumes the entire completion budget before producing visible text, the panel reports that condition and recommends disabling reasoning or increasing the token limit. A separate timeout message explains when generation exceeds the configured response time.
...
| Provider starts but panel reports no visible answer | Reasoning consumed the token budget.                             | Set Reasoning effort to None or increase Maximum output tokens.                                       |
